...
// itself and set the runtime pause flag. Path.Run catches the sentinel
...
type PathOptions struct {
	...
	// InitialWait is the pending wait state the path was checkpointed
	// on, if any. Populated by the engine when a suspended path is
	...
	// InitialPauseRequested seeds the runtime pause flag when a path
	// is being reconstructed from a checkpoint with PauseRequested=true.
	// The path will re-park at its first step boundary unless the
	// flag is cleared via UnpausePath before Run is called.
...
// requestPause marks the path as paused. The pause will take effect at
// the next step boundary in Path.Run. Safe to call from any goroutine.
func (p *Path) requestPause(reason string) {
	p.pauseMu.Lock()
...
// goroutine. Has no effect if the path has already observed the flag
// and exited.
func (p *Path) clearPause() {
	p.pauseMu.Lock()
	defer p.pauseMu.Unlock()
	p.paused = false
	p.pauseReason = ""
...
func (p *Path) checkPause() (bool, string) {
	p.pauseMu.Lock()
	defer p.pauseMu.Unlock()
...
func (p *Path) Run(ctx context.Context) error {
	...
	for {
		...
		// Check for a pending pause request. A paused path emits a
		// PauseRequest snapshot and exits cleanly — the orchestrator
		// persists Status=Paused and removes the path from
		// activePaths, just like a wait-unwind. On resume, the path
		// will be reconstructed with InitialPauseRequested=true and
		// re-park here until UnpausePath clears the flag.
...
//   - Evaluate the topic template against current path state. The resolved
...
//   - If the path is being replayed after a resume, reuse the original
...
//     state; Path.Run translates the sentinel into a WaitRequest snapshot
//     and the orchestrator hard-suspends the path.
func (p *Path) handleWaitSignalStep(ctx context.Context, step *Step) (any, error) {
	cfg := step.WaitSignal
	if cfg == nil {
	...
	if sig != nil {
		...
		// advance past the step. Path.Run treats a non-error return as
		// normal progress; processPathSnapshot will clear PathState.Wait.
		if cfg.Store != "" {
			varName := strings.TrimPrefix(cfg.Store, "state.")
			p.state.SetVariable(varName, sig.Payload)
		}
		if p.formatter != nil {
			p.formatter.PrintStepOutput(step.Name, sig.Payload)
		}
...
// Path.Run continues the loop; the top-of-loop pause check picks up
// the flag and emits a Paused snapshot referencing the already-advanced
// successor step. On unpause + resume, the path continues at that
...
// unpause the path completes. Multiple matching edges are rejected
// because a pause is a single-choice gate, not a branch point.
func (p *Path) handlePauseStep(ctx context.Context, step *Step) (any, error) {
	cfg := step.Pause
	if cfg == nil {
	...
	if p.formatter != nil {
		p.formatter.PrintStepStart(step.Name, "pause")
	}
	...
	// terminal hold: after unpause the path has no successor and
	// completes. We leave p.currentStep at the pause step so the
	// Paused snapshot references it; on unpause + resume, the path
