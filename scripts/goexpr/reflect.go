...
// Return signatures supported: (), (T), (T, error).
func callFunction(name string, fn any, args []any) (any, error) {
	...
	in, err := buildCallArgs(name, ft, args)
	if err != nil {
		return nil, err
	}
...
func buildCallArgs(name string, ft reflect.Type, args []any) ([]reflect.Value, error) {
	if ft.IsVariadic() {
		fixed := ft.NumIn() - 1
		if len(args) < fixed {
		...
		for i := 0; i < fixed; i++ {
			v, err := convertArg(name, i, args[i], ft.In(i))
			if err != nil {
				return nil, err
			}
			in[i] = v
		}
		variadicElem := ft.In(fixed).Elem()
	...
	if len(args) != ft.NumIn() {
		return nil, fmt.Errorf("%w: %q expects %d args, got %d", ErrEvaluate, name, ft.NumIn(), len(args))
	}
	in := make([]reflect.Value, len(args))
	for i, a := range args {
		v, err := convertArg(name, i, a, ft.In(i))
		if err != nil {
			return nil, err
		}
		in[i] = v
...
func convertArg(name string, idx int, value any, want reflect.Type) (reflect.Value, error) {
	...
	// but callers often declare fn(int) / fn(float32) / etc.
	if want.Kind() >= reflect.Int && want.Kind() <= reflect.Float64 && rv.Type().ConvertibleTo(want) {
