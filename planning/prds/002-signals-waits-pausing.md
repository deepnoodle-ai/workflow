...
**Resolved during Phase 3 implementation** (see Phase 3 commits on `signals-pause`):

- ~~`workflow.Wait` context cancellation~~ — Resolved: yes. `workflow.Wait` checks `ctx.Err()` before touching the store and returns the context error unchanged. Tested via `TestWaitCancelDuringWait`. The declarative `WaitSignal` step handler does not currently re-check `ctx.Err()` at every tick because hard-suspend exits the run loop naturally when the context is cancelled upstream; Phase 5 soft-suspend may revisit this.
- ~~Error type for `workflow.Wait` timeout~~ — Resolved: reuse `ErrorTypeTimeout`. `ClassifyError` now recognises `ErrWaitTimeout` and returns a `WorkflowError` of type `timeout`, so catch handlers with `ErrorEquals: ["timeout"]` route wait timeouts the same as retry timeouts. Tested via `TestWaitImperativeTimeoutWithCatch`.
- ~~Double-checkpoint on wait-unwind~~ — Resolved: `executeActivity` short-circuits logging, checkpointing, and the `AfterActivityExecution` callback when the activity returns a wait-unwind. `processPathSnapshot` is now the single source of truth for the suspension checkpoint.
- ~~Retry/catch bypass for wait-unwind should be explicit~~ — Resolved: explicit `IsWaitUnwind(err)` guards added at the top of `executeStep` (pre-catch), `executeStepWithRetry` (inside the retry loop), and `MatchesErrorType`. Mirrors the existing `ErrFenceViolation` bypass. Tested via `TestWaitRetryBypass` and `TestWaitCatchBypass`.
- ~~Context interface vs. type assertion for `workflow.Wait`~~ — Resolved: exported a `SignalAware` side interface with `SignalStore() SignalStore`, `ExecutionID() string`, and `PendingWait() *WaitState` methods. `*executionContext` is the only built-in implementation; `workflow.Wait` asserts against the interface so wrappers / decorators that preserve `SignalAware` continue to work. `PendingWait` was added so the imperative `Wait` can reuse the absolute deadline from a prior suspension's checkpoint rather than restarting the clock on replay.
...
- **Runner handling of a `Suspended` result.** The Runner integration section (Phase 5) talks about `SuspendAfter` but doesn't define the basic Phase-3 handshake: the Runner calls `Execute`, the result comes back `Suspended`, now what? **Decided for Phase 3**: option (a) — `Execute`/`ExecuteOrResume` returns the `ExecutionResult` with `Status = Suspended` and a populated `SuspensionInfo`; the caller is responsible for scheduling the resume. The Runner is uninvolved. Option (b) — Runner parks internally and wakes on `SignalStore.Subscribe` delivery — is deferred to Phase 5 along with `SuspendAfter`. This keeps Phase 3 consumable without any Runner changes.
