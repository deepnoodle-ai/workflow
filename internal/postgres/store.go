...
func (s *Store) CreateTask(ctx context.Context, t *domain.TaskRecord) error {
	...
	`,
		...
		t.Status,
		specJSON,
		nullString(t.WorkerID),
		t.VisibleAt,
		nullTime(t.LastHeartbeat),
		resultJSON,
		t.CreatedAt,
...
func (s *Store) ClaimTask(ctx context.Context, workerID string) (*domain.TaskClaimed, error) {
	...
	claimed.HeartbeatInterval = s.config.HeartbeatInterval
	claimed.LeaseExpiresAt = time.Now().Add(s.config.LeaseTimeout)
